...
const CSAC_DB_PASS = 'passwd123456';
...
{
    return csac_fetch_one("SELECT {$columns} FROM chat_user WHERE id = ?", 'i', $uid);
}

function csac_room(int $roomId, string $columns = '*'): ?array
{
    return csac_fetch_one("SELECT {$columns} FROM chat_room WHERE id = ?", 'i', $roomId);
}
...
function csac_api_user_get_info(): void
{
    ...
    $user = csac_user($viewUid, 'id, avatar, nickname, username, last_active');
    if (!$user) {
    ...
    if (!$isSelf) {
        ...
        if ($rel) {
            ...
            if ($status === 1) {
                ...
                $isBlocked = (int)($rel['delete_by'] ?? 0) === $viewUid;
                $canAddFriend = false;
            }
        }
...
function csac_api_user_update_profile(): void
{
    csac_require_method('POST');
    $uid = requireLogin();
    $user = csac_user($uid, 'id, username, nickname, pwd, avatar');
    if (!$user) {
        response_json(['success' => false, 'message' => '用户不存在'], 404);
    }
...
function csac_api_friend_get_common_groups(): void
{
    ...
    $rows = csac_fetch_all(
        'SELECT DISTINCT cr.id, cr.id AS room_id, cr.room_name, cr.invite_code, cr.intro
         FROM chat_room cr
...
function csac_api_group_edit_info(): void
{
    ...
    if ($action !== '') {
        ...
        foreach (['room_name', 'intro', 'notice'] as $field) {
    ...
    response_json(['success' => true, 'message' => '修改成功']);
}
...
function csac_api_group_disband(): void
{
    ...
    if ((int)($room['is_disband'] ?? 0) === 1) {
        response_json(['success' => false, 'message' => '群组已解散']);
    }
...
function csac_api_message_get_group_msg(): void
{
    ...
    $order = $beforeId > 0 || $afterId > 0 ? 'ASC' : 'DESC';
    $rows = csac_fetch_all(
        'SELECT m.id, m.uid, m.nickname, m.content, m.msg_type, m.voice_duration, m.add_time, m.reply_to, m.mention_uids, u.avatar,
    ...
    foreach ($rows as $row) {
        $sender = (int)$row['uid'];
        $msgTime = strtotime((string)$row['add_time']) ?: 0;
...
function csac_api_message_recall_msg(): void
{
    ...
    if ($type === 'group') {
        ...
        requireGroupMember($roomId, $uid);
        $msg = csac_fetch_one('SELECT uid, add_time FROM chat_msg WHERE id = ? AND room_id = ?', 'ii', $msgId, $roomId);
        if (!$msg) {
        ...
        csac_execute('DELETE FROM chat_msg WHERE id = ? AND room_id = ?', 'ii', $msgId, $roomId);
...
function csac_api_essence_get(): void
{
    ...
    requireGroupMember($roomId, $uid);
    $rows = csac_fetch_all(
        'SELECT m.id, m.uid, m.nickname, m.content, m.msg_type, m.voice_duration, m.add_time, u.avatar,
